...
n$geo_id <- as.character(n$geo_id)
alt_polygons$geo_id <- as.character(alt_polygons$geo_id)
bg_polygons$geo_id <- as.character(bg_polygons$geo_id)
zip_polygons$geo_id <- as.character(zip_polygons$geo_id)
...
geometry_combined <- bind_rows(n,bg_polygons,zip_polygons,alt_polygons)
write_sf(geometry_combined,"finalData/geometry_polygons.geojson")
...
```{r}
census_wide_bg <- census %>% 
  filter(geo_type == "Census Block Group" & year == 2019) %>% 
  pivot_wider(id_cols=c(geo_id,geo_type,year) , names_from = var, values_from = value) 


indicators_wide_bg <- pillars %>% 
  filter(geo_type == "Census Block Group" & year == 2020) %>% 
  pivot_wider(id_cols=c(geo_id,geo_type,year) , names_from = var, values_from = value) 

indicators_wide_bg_delta <- pillars %>% 
  filter(geo_type == "Census Block Group" & year == 2020) %>% 
  pivot_wider(id_cols=c(geo_id,geo_type,year) , names_from = var, values_from = delta_percent, names_prefix = "delta_") 

is_all_numeric <- function(x) {
  !any(is.na(suppressWarnings(as.numeric(na.omit(x))))) & is.character(x)
}
indicators_wide_bg <- indicators_wide_bg %>% 
  mutate_if(is_all_numeric,as.numeric) %>% mutate(geo_id = as.character(geo_id))


wide_bg <- left_join(bg_polygons, census_wide_bg, by=c("geo_id","geo_type"))
wide_bg <- left_join(wide_bg, indicators_wide_bg, by=c("geo_id","geo_type"))
wide_bg <- left_join(wide_bg, indicators_wide_bg_delta, by=c("geo_id","geo_type"))

write_sf(wide_bg,"finalData/test_breaks.geojson")
