...
#set proijection for area, in this case NY state plane NAD83
prj <- 2830
...
* [Yost, K., Perkins, C., Cohen, R. et al. Socioeconomic status and breast cancer incidence in California for different race/ethnic groups. Cancer Causes Control 12, 703–711 (2001)](https://doi.org/10.1023/A:1011240019516)
...
boundary <- sf::read_sf("data/geospatial_data/COB_municipal_boundary.shp") %>% st_transform(4326)
mapview(boundary)
...
```{r census_boundaries}
bg <- get_acs(state = state, geography = "block group", variables = c("B01001_001","B11016_001"), geometry = TRUE, output="wide") %>% st_transform(4326)
tr <- get_acs(state = state, geography = "tract", variables = c("B01001_001","B11016_001"), geometry = TRUE, output="wide") %>% st_transform(4326)
...
alt <- sf::read_sf("data/geospatial_data/COB_council_distrcits.shp") %>% st_transform(4326)
...
imp.bg.long <- select(imp.bg.long,District,var,value)
imp.neigh.long <- imp.bg.long %>% 
  group_by(District,var) %>%  #change depending on alt polygon name
  summarise(value=sum(value))


alt_polygons <- alt %>% 
  left_join(imp.neigh.long,by="District") %>% #change depending on alt polygon name
  select(District,var,value)#change depending on alt polygon name

alt_polygons$alt_polygon_id <- alt_polygons$District #change depending on alt polygon name
...
alt_polygons <- distinct(select(alt_polygons,alt_polygon_id),.keep_all = TRUE)
...
boundary.p$id <- boundary.p$Name # set id
...
write_sf(alt_polygons,"finalData/alternate.geojson")
write_sf(bg_polygons,"finaldata/block_group.geojson")


overall <- left_join(bg_data,select(bg,GEOID,B01001_001E),by="GEOID")
overall <- st_as_sf(overall)
overall$area_bg <- st_area(overall)
overall <- st_intersection(overall,select(boundary.p,id))
...
colnames(n)[1]<-"geo_id"
write_sf(n,"finaldata/service_area.geojson")
```
...
pillars$geo_type[(l_bg+1):(l_bg+l_alt)] <- "alternate"
...
pillars$value[which(pillars$var=="pillar1_affordability_score")] <- sample(c("Low","Medium","High"),length(pillars$var[which(pillars$var=="pillar1_affordability_score")]),replace=TRUE)
pillars$delta_percent[which(pillars$units=="character")]<-NA

# This will need to be its own code block customized for each variable
pillars$var_bucket <- sample(c("High","Medium","Low"),length(pillars$geo_id),replace=TRUE)
...
ascending_vars <- c("pillar1_affordability_score", #ordering so higher is bad
                    "pillar1_cutoff",
                    "pillar1_hbi",
                    "pillar1_ppi",
                    "pillar3_breaks",
...
descending_vars <- c( #ordering so higher is good
                     "pillar1_cap", 
                     "pillar1_connections_sewer",
                     "pillar1_connections_water",
                     "pillar2_diversity_1",
...
pillars <- pillars %>% mutate(var_rank_order = case_when(
  var %in% ascending_vars ~ "ascending",
  var %in% descending_vars ~ "descending")) 

pillars$value_character <- pillars$value
pillars$value_character[which(pillars$units != "character")] <- NA

pillars$value[which(pillars$var == "pillar1_affordability_score" & pillars$value == "High")] <- "3"
pillars$value[which(pillars$var == "pillar1_affordability_score" & pillars$value == "Medium")] <- "2"
pillars$value[which(pillars$var == "pillar1_affordability_score" & pillars$value == "Low")] <- "1"

pillars$value <- as.numeric(pillars$value)


pillars_asc <- pillars %>% 
  filter(var_rank_order == "ascending") %>%
  group_by(year,geo_type,var) %>%
  add_tally() %>%
  arrange(.,value,value_character, .by_group=TRUE) %>% 
  mutate(var_rank = order(value, value_character))

pillars_desc <- pillars %>% 
  filter(var_rank_order == "descending") %>%
  group_by(year,geo_type,var) %>%
  add_tally() %>%
  arrange(.,value,value_character, .by_group=TRUE) %>% 
  mutate(var_rank = order(value, value_character, decreasing = TRUE))

pillars <- bind_rows(pillars_asc,pillars_desc)
pillars <- select(pillars,
                  geo_id,
                  geo_type,
                  year,
                  var,
...
pillars <- pillars %>% ungroup() %>% arrange(geo_type,var,var_rank,year)
