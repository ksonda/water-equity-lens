...
# st_write(tracts,"../data/tracts.geojson", append=FALSE)
# write.csv(st_drop_geometry(tracts),"../data/tracts.csv")
...
# st_write(bg,"../data/blockgroups.geojson", append=FALSE)
# write.csv(st_drop_geometry(bg),"../data/blockgroups.csv")
...
bg$perc_white = 100*bg$pop_race_whiteE/bg$pop_race_countE
tracts$perc_white = 100*tracts$pop_race_whiteE/tracts$pop_race_countE

bg$housing_unit_median_year_built_overallE[which(bg$housing_unit_median_year_built_overallE==0)] <-NA

tracts$housing_unit_median_year_built_overallE[which(tracts$housing_unit_median_year_built_overallE==0)] <-NA

```

```{r, results="asis", echo=FALSE, message=FALSE}
bg.white.map <- mapview(select(bg,perc_white),
                        zcol="perc_white",
...
tracts.white.map <- mapview(select(tracts,perc_white),
                            zcol="perc_white",
...
bg.yearbuilt.map <- mapview(select(bg,housing_unit_median_year_built_overallE),
...
tracts.yearbuilt.map <- mapview(select(tracts,housing_unit_median_year_built_overallE),
                        zcol="housing_unit_median_year_built_overallE",
...
## Develop and simulate Water Equity Lens indicators
...
One approach was developed for AWWA, NACQA, and WEF. It is elaborated in [this report](https://raw.githubusercontent.com/ksonda/water-equity-lens/draft1/literature/DevelopingNewFrameworkForAffordabilityReport.pdf?token=AKQHTNUSCBCKR3XAIOECHDTAE3YYG) 

![HBI/PPI](https://raw.githubusercontent.com/ksonda/water-equity-lens/draft1/doc/tab32.png?token=AKQHTNSF6UUWMCCPOFQY4VDAE3WJY)

Its 4-level qualitative scale could be easily converted into a color scale/gage/stoplight, and is easily calculated from US Block Group. 

* Approach 2: Proportion renting HH whose bills are likely above 10% of their income net of gross rent/ housing costs. 

* Whether there is a CAP program
* Ratio of HH in CAP program to HH on TANF/SNAP
* Proportion of HH cut off due to nonpayment at least once/twice

### Pillar 2: Workforce and Economic Development Benefits

### Pillar 3: Resilience and Investment

* Proprtion structures with lead service lines
